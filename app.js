...
function setAuthenticated(user) {
  ...
  if (assistantLauncher) assistantLauncher.hidden = false;
  document.querySelectorAll('[data-management-only]').forEach(element => { element.style.display = ['hr', 'manager'].includes(user.role) ? '' : 'none'; });
}
function signOut(showMessage = true) { currentUser = null; localStorage.removeItem('dayflow-token'); appShell.classList.remove('authenticated'); authScreen.style.display = ''; if (assistantPanel) { assistantPanel.classList.remove('open'); assistantPanel.hidden = true; } const assistantLauncher = document.querySelector('#assistant-launcher'); if (assistantLauncher) assistantLauncher.hidden = true; if (showMessage) { authError.textContent = 'You have been signed out.'; } }
...
function setTheme(theme) { document.body.classList.toggle('dark-mode', theme === 'dark'); localStorage.setItem('dayflow-theme', theme); const themeButton = document.querySelector('#theme-button'); if (themeButton) themeButton.textContent = theme === 'dark' ? '☀' : '◐'; }
