...
const viewMeta = { attendance: ['Attendance', 'Your time, clearly accounted for.'], leave: ['Leave requests', 'Plan time away without the paperwork.'], employees: ['People', '48 people making good work happen.'], payroll: ['Payroll', 'Your compensation, always within reach.'], profile: ['My profile', 'The details that make you, you.'], settings: ['Settings', 'Make Dayflow work your way.'] };
...
async function renderView(name) {
  ...
  try {
    if (name === 'attendance') {
      ...
      genericContent.innerHTML = table(records.map(record => `<tr><td>${record.work_date}</td><td>${record.check_in || '--'}</td><td>${record.check_out || '--'}</td><td><span class="pill ${record.status === 'Present' ? '' : 'pending'}">${record.status}</span></td></tr>`).join(''), ['Date', 'Check in', 'Check out', 'Status']);
    } else if (name === 'leave') {
      const leaves = await api('/leaves');
      genericContent.innerHTML = `<div class="panel-heading"><h2>My requests</h2><button class="primary-button" data-open-modal="leave-modal">＋ New request</button></div>${table(leaves.map(item => `<tr><td>${item.type}</td><td>${item.start_date} - ${item.end_date}</td><td>${item.days}</td><td><span class="pill ${item.status === 'Pending' ? 'pending' : ''}">${item.status}</span></td></tr>`).join(''), ['Type', 'Dates', 'Days', 'Status'])}`;
    } else if (name === 'payroll') {
      const payroll = await api('/payroll');
      genericContent.innerHTML = `<div class="panel-heading"><div><p class="eyebrow">CURRENT PAY PERIOD</p><h2>Salary overview</h2></div><span class="pill">Read only</span></div><div class="stat-grid" style="margin-top:25px"><article class="stat-card"><div class="stat-label">NET PAY</div><strong>$${payroll.net_pay.toLocaleString()} <small>this month</small></strong><p>After taxes and deductions</p></article><article class="stat-card"><div class="stat-label">GROSS PAY</div><strong>$${payroll.gross_pay.toLocaleString()} <small>this month</small></strong><p>Payroll managed by HR</p></article></div>`;
      ...
      genericContent.innerHTML = table(employees.map(user => `<tr><td>${user.name}</td><td>${user.department}</td><td>${user.jobTitle}</td><td>${user.role}</td></tr>`).join(''), ['Employee', 'Department', 'Role', 'Access']);
    } else {
...
async function start() {
  if (canUseApi) { try { currentUser = await api('/me'); } catch { try { const session = await api('/auth/signin', { method: 'POST', body: JSON.stringify({ email: 'alex.smith@dayflow.co', password: 'dayflow123' }) }); localStorage.setItem('dayflow-token', session.token); currentUser = session.user; } catch { showToast('Backend is unavailable'); } } }
