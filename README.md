...
From the manager or HR People view, the `Payroll` action updates gross pay and deductions and stores the calculated net pay. The `Review` action stores an employee goal, progress percentage, feedback, and commitment status. All changes are persisted in `dayflow-db.json` and are reflected when the employee logs in again.
